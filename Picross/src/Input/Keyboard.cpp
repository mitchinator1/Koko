...
namespace Input
{
	Keyboard::Keyboard(std::shared_ptr<Display>& display, const std::string& filename)
		: InputBase(display)
	{
	...
	{
		bool capital = false;
		if (glfwGetKey(s_Display->Window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS || glfwGetKey(s_Display->Window, GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS)
		{
			capital = true;
		}
